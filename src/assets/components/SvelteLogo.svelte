<script lang="ts">
  ...
  function onMouseMove(e: MouseEvent) {
    ...
    const distanceFromCentre = distanceBetween({ x, y }, centre)
    const maxDistance = distanceBetween(centre, { x: windowWidth, y: windowHeight })
    const size = 5 - (4 * distanceFromCentre) / maxDistance
...
<style>
  ...
  svg {
    ...
    transform: rotateZ(var(--rotation)) scale(var(--scale));
  }
</style>
